...
// addTestFlow adds a test flow directly to the backend for testing purposes.
// Creates a session if needed.
func (b *CollyBackend) addTestFlow(flow *CrawlFlow) {
	b.mu.Lock()
	defer b.mu.Unlock()
	...
	session.mu.Lock()
	session.flowsByID[flow.ID] = flow
	session.flowsOrdered = append(session.flowsOrdered, flow)
	session.mu.Unlock()
